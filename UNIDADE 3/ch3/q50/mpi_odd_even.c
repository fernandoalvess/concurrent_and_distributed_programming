...
#  endif
   ...
   if (my_rank == 0) printf("O codigo a ser cronometrado levou %e segundos\n", elapsed);
...
/*
 * MODIFICADO: Recebe ponteiros duplos para poder alterar os ponteiros originais.
 */
void Merge_low(
      int** my_keys_pp,     /* in/out    */
      int* recv_keys,      /* in        */
      int** temp_keys_pp,   /* scratch   */
      int   local_n) {
   int m_i, r_i, t_i;
   int* temp;
   
   m_i = r_i = t_i = 0;
   // O loop de mesclagem para o buffer temporário
   while (t_i < local_n) {
   ...
   // Em vez de memcpy, trocamos os ponteiros.
   temp = *my_keys_pp;
 ...
 */
void Merge_high(
        int** local_A_pp, /* in/out */
        int* temp_B,     /* in */
        int** temp_C_pp,  /* scratch */
        int   local_n) {
   int ai, bi, ci;
   int* temp;
