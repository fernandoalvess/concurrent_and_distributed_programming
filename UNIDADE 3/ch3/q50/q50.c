 ...
 *
 * Compile:  mpicc -g -Wall -o q50 q50.c
 * Run:
 *    mpiexec -n <p> q50 <g|i> <global_n> 
...
void Merge_low(int** my_keys_pp, int recv_keys[], int** temp_keys_pp, int local_n);
...
void Sort(int** local_A_pp, int local_n, int my_rank, 
         int p, MPI_Comm comm);
void Odd_even_iter(int** local_A_pp, int* temp_B, int** temp_C_pp,
...
#  endif
   ...
   //printf("I am process %d and my local time was: %.4f\n", my_rank, local_elapsed);
...
#  endif
   ...
   for (phase = 0; phase < p; phase++)
      Odd_even_iter(local_A_pp, temp_B, &temp_C, local_n, phase, 
...
void Odd_even_iter(int** local_A_pp, int* temp_B, int** temp_C_pp,
   ...
   if (phase % 2 == 0) {
      if (even_partner >= 0) {
         MPI_Sendrecv(*local_A_pp, local_n, MPI_INT, even_partner, 0, 
            temp_B, local_n, MPI_INT, even_partner, 0, comm,
            &status);
         if (my_rank % 2 != 0)
            Merge_high(local_A_pp, temp_B, temp_C_pp, local_n);
         else
            Merge_low(local_A_pp, temp_B, temp_C_pp, local_n);
      }
   } else { /* odd phase */
      if (odd_partner >= 0) {
         MPI_Sendrecv(*local_A_pp, local_n, MPI_INT, odd_partner, 0, 
