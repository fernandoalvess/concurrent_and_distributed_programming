 ...
 *           a message.
 *
 * Compile:  mpicc -g -Wall -o q39 q39.c -lm
 * Usage:    mpiexec -n<number of processes> ./q39
 *
 * Input:    None
...
int main(void) {
   ...
   Read_vector(local_vector_1, local_n_vector, n_vector, "X", my_rank, MPI_COMM_WORLD);

   Print_vector(local_vector_1, local_n_vector, n_vector, "X", my_rank, MPI_COMM_WORLD);
   ...
   Print_vector(local_vector_1, local_n_vector, n_vector, "Vetor X multiplicado pelo escalar", my_rank, MPI_COMM_WORLD);

   Allocate_vector(&local_vector_2, local_n_vector, MPI_COMM_WORLD);

   Read_vector(local_vector_2, local_n_vector, n_vector, "Y", my_rank, MPI_COMM_WORLD);

   Print_vector(local_vector_2, local_n_vector, n_vector, "Y", my_rank, MPI_COMM_WORLD);
