/* File:     mpi_vector_39.c
 *
 * Purpose:  A program in which multiple MPI processes try to print 
 *           a message.
 *
 * Compile:  mpicc -g -Wall -o mpi_vector_39 mpi_vector_39.c -lm
 * Usage:    mpiexec -n<number of processes> ./mpi_vector_39
 *
 * Input:    None
...
int main(void) {
   ...
   Read_vector(local_vector_1, local_n_vector, n_vector, "ONE", my_rank, MPI_COMM_WORLD);

   Print_vector(local_vector_1, local_n_vector, n_vector, "ONE", my_rank, MPI_COMM_WORLD);
   ...
   Print_vector(local_vector_1, local_n_vector, n_vector, "THE VECTOR ONE MULTIPLIED BY THE SCALAR", my_rank, MPI_COMM_WORLD);

   //START VECTOR TWO
   Allocate_vector(&local_vector_2, local_n_vector, MPI_COMM_WORLD);

   Read_vector(local_vector_2, local_n_vector, n_vector, "TWO", my_rank, MPI_COMM_WORLD);

   Print_vector(local_vector_2, local_n_vector, n_vector, "TWO", my_rank, MPI_COMM_WORLD);
