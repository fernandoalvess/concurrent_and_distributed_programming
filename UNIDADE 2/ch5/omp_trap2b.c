...
int main(int argc, char* argv[]) {
   ...
   int     thread_count;
   double start, finish, time_result = 0;
...
#  pragma omp parallel num_threads(thread_count) \
      reduction(+: global_result) default (none) shared(a, b, n, time_result) private(start, finish)
      {
#  pragma omp barrier
   start = omp_get_wtime();
   ...
   printf("Thread %d Time: %f\n", thead_number, (finish - start));

   if (time_result < (finish - start)) {
#     pragma omp critical  
